...
function tacharJugada(jugador, jugada) {
  ...
  function jugar() {
    ...
    // Guardar los nombres de los jugadores en el arreglo jugadores
    jugadores[0].nombre = jugador1;
    jugadores[1].nombre = jugador2;
  
    // Escuchar los eventos click en los botones de la grilla de anotación
  ...
  // Función para anotar los puntos correspondientes a una jugada
  function anotarPuntos(jugada) {
    let puntos = calcularPuntosJugada(jugada)
    jugadores[turno].puntos[jugada] = puntos;
  
    // Actualizar los puntos del jugador en la grilla de anotación
    actualizarPuntos(jugadores[turno].nombre, jugada, puntos);
  
    // Cambiar el turno
    turno = turno === 0 ? 1 : 0;
  
    // Deshabilitar las celdas que ya fueron anotadas por ambos jugadores
    deshabilitarCeldas();
  
    // Verificar si ya se completaron todas las jugadas
    if (jugadores[0].puntos.length === 11 && jugadores[1].puntos.length === 11) {
      // Mostrar el ganador de la partida
      let ganador = obtenerGanador();
      alert(`¡${ganador} ha ganado la partida!`);
    } else {
      // Mostrar el turno del jugador en la grilla de anotación
      document.getElementById("nombre-jugador1").classList.toggle("turno-actual");
      document.getElementById("nombre-jugador2").classList.toggle("turno-actual");
    }
  }
